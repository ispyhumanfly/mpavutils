...
sub new {
    ...
    if ( $^O =~ /^mswin/i ) {
        ...
        $params{'mplayer'} = 'bin\mplayer.exe'
            if -e 'bin\mplayer.exe' or die "Unable to find mplayer.exe!\n";

        # mencoder...
        $params{'mencoder'} = 'bin\mencoder.exe'
            if -e 'bin\mencoder.exe' or die "Unable to find mencoder.exe!\n";
    }
    ...
    # if the input is for an existing mplayer output file...
        ...
        open my $fh_input, '<', $params{input} or die "Unable to open the input specified: $!\n";
