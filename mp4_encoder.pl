...
unless ( $file_output =~ /\.mp4|\.avi/ ) {
    encode_status( "valid output file names must have the .avi or .mp4 extension!\n", "first" );
    exit;
}
...
sub sort_arguments {
    ...
    foreach $argument (@ARGV) {
        ...
        if ( $argument =~ /--screen-/ ) {
            ...
            unless ( $b =~ /wide|full/ ) {
                encode_status( "valid screen types are wide and full screen only...", "first" );
                exit;
            }
            elsif ( $b eq "wide" ) {
                $aspect = 16 / 9;
        ...
        if ( $argument =~ /--video-/ ) {
            ...
            unless ( $v_codec =~ /x264|ffmpeg|xvid/ ) {
                encode_status( "valid video codecs are x264, ffmpeg or xvid only...", "first" );
                exit;
            }
        }
        ...
        if ( $argument =~ /--audio-/ ) {
            ...
            unless ( $a_codec =~ /faac|mp3|ac3/ ) {
                encode_status( "valid audio codecs are faac, mp3 or ac3 only...\n", "first" );
                exit;
            }
...
#            }
        ...
        if ( $argument =~ /--language-/ ) {
            ...
            unless ( $lang_track >= 128 ) {
                encode_status( "valid language tracks are between 128 and 135 only...\n", "first" );
                exit;
            }
        }
        ...
        if ( $argument =~ /--filesize-/ ) {
            ...
            unless ( ( $filesize >= 10 ) or ( $filesize <= 3000 ) ) {
                encode_status( "valid file sizes are between 25MB and 3000MB's only...\n", "first" );
                exit;
            }
        }
        ...
        if ( $argument =~ /--subtitle-/ ) {
            ...
            unless ( ( $sub_track >= 0 ) or ( $sub_track <= 31 ) ) {
                encode_status( "valid subtitle tracks are between 0 and 30 only...\n", "first" );
                exit;
            }
        }
    ...
    if ( ( $aspect != 16 / 9 ) and ( $aspect != 4 / 3 ) or ( $lang_track eq "" ) or ( $filesize eq "" ) or ( $a_codec eq "" ) or ( $a_quality eq "" ) or ( $v_codec eq "" ) ) {
        encode_status( "missing manditory switch! try --help for usage information...\n", "first" );
        exit;
    }

    calculate_settings( $aspect, $v_codec, $a_codec, $a_quality, $lang_track, $filesize, $signature, $change_owner, $sub_track, $sub_name, $test_dvd, $log, $crop );
...
sub calculate_settings {
    ...
    encode_status( "creating temporary file to help determine video settings...", "first" );
    open my $CMD, "nice -n 19 mencoder $file_input -ofps 24000/1001 -o temp.avi -oac copy -ovc copy -ss 9:30 -endpos 0:10 1>/dev/tty8 2>/dev/tty8 |" or die "unable to open command: $!";
    close($CMD);

    encode_status("loading mplayer to generate video data...");
    open my $CMD, "nice -n 19 mplayer temp.avi -nosound -vf cropdetect 1>./video.stderr 2>/dev/tty8 |" or die "unable to open command: $!";
    close($CMD);

    # find the mencoder version and build...
    open my $OUTFILE, "<", $video_output or die "unable to open $video_output\: $!";
    @lines = <$OUTFILE>;
    foreach $line (@lines) {
        if ( $line =~ /Software: / ) {
    ...
    open my $CMD, "nice -n 19 mencoder $file_input -ofps 24000/1001 -o frameno.avi -oac copy -ovc frameno 1>./audio.stderr 2>/dev/tty8 |" or die "unable to open command: $!";
    close($CMD);

    # find the frameno.avi detected audio length...
    open my $OUTFILE, "<", $audio_output or die "unable to open $audio_output\: $!";
    @lines = <$OUTFILE>;
    foreach $line (@lines) {
    ...
    close($OUTFILE);
    encode_status( "an audio length of " . ( $audio_length / 60 ) . " minutes has been detected..." );
    open my $CMD, "rm -f audio.stderr frameno.avi /dev/null |" or die "unable to open command: $!";
    close($CMD);

    # determine bitrate information based on audio codec...
    encode_status("calculating the exact bitrate of the $audio_codec audio codec...");

    if ( $audio_codec eq "faac" ) {
        open my $CMD, "nice -n 19 mencoder $file_input -ofps 24000/1001 -o frameno.avi -oac faac -aid $lang -faacopts object=0:tns:quality=$audio_quality -ovc frameno -ss 9:30 -endpos 0:60 1>./audio.stderr 2>/dev/tty8 |" or die "unable to open command: $!";
        close($CMD);
    }
    elsif ( $audio_codec eq "mp3" ) {
        open my $CMD, "nice -n 19 mencoder $file_input -ofps 24000/1001 -o frameno.avi -oac mp3lame -aid $lang -lameopts abr:br=$audio_quality -ovc frameno -ss 9:30 -endpos 0:60 1>./audio.stderr 2>/dev/tty8 |" or die "unable to open command: $!";
        close($CMD);
    }
    elsif ( $audio_codec eq "ac3" ) {
        open my $CMD, "nice -n 19 mencoder $file_input -ofps 24000/1001 -o frameno.avi -oac lavc -aid $lang -lavcopts acodec=ac3:abitrate=$audio_quality -ovc frameno -ss 9:30 -endpos 0:60 1>./audio.stderr 2>/dev/tty8 |" or die "unable to open command: $!";
        close($CMD);
    }
    ...
    open my $OUTFILE, ">>", $bpp_output or die "unable to open $bpp_output\: $!";
    printf $OUTFILE ( "Prescaled picture: %dx%d, AR %.2f\n", $scaled_width, $scaled_height, $picture_ar );
    ...
    for ( $width = 720 ; $width >= 320 ; $width -= 16 ) {
        ...
        printf $OUTFILE ( "${width}x${height}, diff % 3d, new AR %.2f, AR error %.2f%% " . "scale=%d:%d bpp: %.3f\n", $diff, $new_ar, $picture_ar_error, $width, $height, ( $bps * 1000 ) / ( $width * $height * $fps ) );
    }
    close($OUTFILE);
    ...
    open my $CMD, "rm -f video.stderr temp.avi bpp.stderr /dev/null |" or die "unable to open command: $!";
    close($CMD);

    # if --logfile switch is true...
    if ( $log_file eq "true" ) {

        encode_log( "$file_output", "mencoder version  :: $mencoder_version", "first" );
        encode_log( "$file_output", "video title       :: $file_output" );
        encode_log( "$file_output", "video size        :: $file_size\MB" );
        encode_log( "$file_output", "video length      :: " . ( $audio_length / 60 ) . " minutes" );
        ...
        encode_log( "$file_output", "audio codec       :: $audio_codec codec family" );
        encode_log( "$file_output", "audio bitrate     :: $abit\kbps" );

        # if --subtitle- switch selected...
        if ( $subtitle ne "" ) {
            encode_log( "$file_output", "subtitle track    :: $subname ( sub & idx )" );
        }

        encode_log( "$file_output", "bits per pixel    :: $final_bpp" );
        encode_log( "$file_output", "encode strategy   :: 2-pass $codec codec family" );
    ...
    if ( $codec eq "x264" ) {
        $final_options[0] = "subq=5:bframes=4:me=2:cabac:b_adapt:deblock:frameref=4:b_pyramid:qp_step=4:4x4mv:weight_b:chroma_me:qcomp=0.7";
    }
    elsif ( $codec eq "ffmpeg" ) {
        $final_options[0] = "vcodec=mpeg4:mbd=2:trell:v4mv:autoaspect:cmp=256:vqcomp=0.6:mbqmin=2:mbqmax=10:vqdiff=2:vqmin=2:vqmax=6:mpeg_quant:vlelim=-4:vcelim=9:lumi_mask=0.05:dark_mask=0.01:scplx_mask=0.1:tcplx_mask=0.1:qprd:naq";
    }
    elsif ( $codec eq "xvid" ) {
        $final_options[0] = "chroma_opt:vhq=2:gmc:bvhq=1:hq_ac:quant_type=mpeg:lumi_mask:trellis:chroma_me:max_bframes=1";
    }
    ...
    for ( $pass = 1 ; $pass <= 2 ; $pass++ ) {
        ...
        if ( $audio_codec eq "faac" ) {
            $mencoder[0] = "nice -n 19 mencoder $file_input -ofps 24000/1001 -oac faac -aid $final_options[2] -faacopts object=0:tns:quality=$final_options[1]";
        }
        elsif ( $audio_codec eq "mp3" ) {
            $mencoder[0] = "nice -n 19 mencoder $file_input -ofps 24000/1001 -oac mp3lame -aid $final_options[2] -lameopts abr:br=$final_options[1]";
        }
        elsif ( $audio_codec eq "ac3" ) {
            $mencoder[0] = "nice -n 19 mencoder $file_input -ofps 24000/1001 -oac lavc -aid $final_options[2] -lavcopts acodec=ac3:abitrate=$final_options[1]";
        }
        ...
        if ( $pass == 1 ) {
            if ( $codec eq "x264" ) {
                $mencoder[2] = "-x264encopts $final_options[0]:bitrate=$final_options[3]:pass=$pass:turbo=1";
            }
            elsif ( $codec eq "ffmpeg" ) {
                $mencoder[2] = "-lavcopts $final_options[0]:vbitrate=$final_options[3]:vpass=$pass:turbo";
            }
            elsif ( $codec eq "xvid" ) {
                $mencoder[2] = "-xvidencopts $final_options[0]:turbo:pass=$pass";
            }
        }
        ...
        if ( $pass == 2 ) {
            if ( $codec eq "x264" ) {
                $mencoder[2] = "-x264encopts $final_options[0]:bitrate=$final_options[3]:pass=$pass";
            }
            elsif ( $codec eq "ffmpeg" ) {
                $mencoder[2] = "-lavcopts $final_options[0]:vbitrate=$final_options[3]:vpass=$pass";
            }
            elsif ( $codec eq "xvid" ) {
                $mencoder[2] = "-xvidencopts $final_options[0]:bitrate=$final_options[3]:pass=$pass";
            }
        }

        # scaling, cropping and video filters used...
        if ( $auto_crop eq "true" ) {
            $mencoder[3] = "-vf pullup,softskip,crop=$final_options[5],scale=$final_options[4]";
        }
        else {
        ...
        if ( ( $subtitle ne "" ) and ( $test ne "true" ) and ( $pass == 1 ) ) {
            $mencoder[4] = "-vobsubout $final_options[7] -vobsuboutindex 0 -sid $final_options[6]";
        }
...
sub mencoder_command {
    ...
    open my $CMD, "$command_one 1>/dev/tty8 2>/dev/tty8 |" or die "unable to open command: $!";
    close($CMD);

    # pass two...
    encode_status("running second pass of the encode...");
    open my $CMD, "$command_two 1>/dev/tty8 2>/dev/tty8 |" or die "unable to open command: $!";
    close($CMD);

    # clean-up and end note...
    open my $CMD, "rm -f divx2pass.log /dev/null |" or die "unable to open command: $!";
    close($CMD);

    # if --chown- switch selected...
    if ( $file_owner ne "" ) {
        open my $CMD, "chown $file_owner $file_name 1>/dev/tty8 2>/dev/tty8 |" or die "unable to open command: $!";
        close($CMD);
...
sub encode_log {
    ...
    if ( $special eq "first" ) {
        print $OUTFILE " ________________  _______________________________________ _ _   _ \n";
        print $OUTFILE "( mp4_encoder.pl )( $mp4_version created on $mp4_date encode log...\n\n";
    }
...
sub encode_status {
    ...
    if ( $special eq "first" ) {
        system("clear");
        print " ________________  _______________________________________ _ _   _ \n";
        print "( mp4_encoder.pl )( $mp4_version created on $mp4_date preparing for encode...\n\n";
